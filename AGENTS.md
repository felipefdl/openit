...
- Publish: `.github/workflows/publish.yml` on `v*` tags. Native runners, cargo-packager 0.11.8, Apple signing and notarization (`APPLE_CERTIFICATE`, `APPLE_CERTIFICATE_PASSWORD`, `APPLE_SIGNING_IDENTITY`, `APPLE_API_KEY`, `APPLE_API_ISSUER`, `APPLE_API_KEY_CONTENT`), minisign via `PACKAGER_SIGN_PRIVATE_KEY` and `PACKAGER_SIGN_PRIVATE_KEY_PASSWORD`. Uploads a draft GitHub release plus `latest.json`.
- Updater: `crates/app/src/updater.rs`. Endpoint `https://github.com/felipefdl/openit/releases/latest/download/latest.json`. `OpenIt > Check for Updates...` opens the Updates modal in the current window. `auto_check_updates` (default off) checks once after launch in packaged builds; debug builds skip that automatic check.
